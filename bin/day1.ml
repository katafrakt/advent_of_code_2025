...
type mode = FinalPosition | WithIntermediate

let process_line (pointer : int) (zeros : int) (line : char list) (mode : mode)
    : int * int =
  match line with
  | dir :: rest ->
      ...
      let new_zeros =
        match (mode, raw_value, new_pointer, pointer) with
        ...
        | WithIntermediate, _, _, _ -> (abs raw_value / 100) + 1
      in
...
let process (mode : mode) : unit =
  ...
  |> List.fold_left
       (fun acc line -> process_line (fst acc) (snd acc) line mode)
       (50, 0)
